...
switch ($_GET['op'])
{
    case 'guardaryeditar':
        if (empty($id)) {
            $rspta = $pagoccf->insertar($fecha_abono,$comentario,$saldo,$abono,$nuevo_saldo,$id_forma_pago,$id_banco,$id_ccf,$id_user);
            echo $rspta;
        } else {
            $rspta = $pagoccf->editar($id,$fecha_abono,$comentario,$saldo,$abono,$nuevo_saldo,$id_forma_pago,$id_banco,$id_ccf,$id_user);
            echo $rspta;
        }
        break;

    case 'listar':
        $rspta = $pagoccf->listar();
        $data = Array();
    ...
    //Muestra el detalle de CCF a abonar
    case 'mostrar':
        $rspta = $pagoccf->mostrar($id);
    ...
    case 'actualizarEstado':
        $id=$_POST['id'];
        $rspta = $pagoccf->actualizarEstado($id); 
        echo $rspta;
        break;

    case 'mostrarTotalPendiente':
        $rspta = $pagoccf->mostrarTotalPendiente();
        /*$data = Array(
            "total" => $rspta
        );*/
