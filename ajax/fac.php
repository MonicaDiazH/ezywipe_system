...
$id_fac_anular = isset($_POST["id_fac_anular"]) ? limpiarCadena($_POST["id_fac_anular"]) : "";
...
switch ($_GET['op']) {
    ...
    case 'guardaryeditar':
        if (empty($id)) {
            $rspta = $fac->insertar($fecha, $referencia, $total_letras, $total_ajena, $total_no_sujeta,
            ...
            $rspta = $fac->editar($id, $fecha, $referencia, $total_letras, $total_ajena, $total_no_sujeta,
                $total_exentas, $total_gravadas, $venta_total, $sub_total, $total_general,
                $costo_total, $correlativo, $id_condicion_pago, $id_cliente, $id_user);
    ...
    case 'listar':
        ...
        while ($reg = $rspta->fetch_object()) {
            ...
            if ($estado == 1) {
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <li><a onclick="mostrarFAC(' . $reg->id . ')"><i class="fa fa-pencil"></i>Editar</a></li>
                    </ul>
                </div>
                ';
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <li><a href="../ajax/printProformaFAC.php?id=' . $reg->id . '" target="_blank" ><i class="fa fa-print"></i>Proforma</a></li>
                        <li><a href="../ajax/printFAC.php?id=' . $reg->id . '" target="_blank" ><i class="fa fa-print"></i>FAC</a></li>
            ...
            $data[] = array(
                ...
                "7" => $estado_print,
                "8" => $opciones
            );//onclick="imprimirFAC('.$reg->id.')" || <a onclick="mostrar('.$reg->id.')" data-toggle="tooltip" title="Editar"><i class="fa fa-edit" ></i></a>&nbsp;&nbsp;&nbsp;
        }
        $results = array(
            "sEcho" => 1,
    ...
    case 'totalesFAC':
        $rspta = $fac->totalesFAC($id);
        echo json_encode($rspta);
        break;

    case 'anular':
        $rspta = $fac->anular($id_fac_anular, $comentario, $id_anulacion);
        echo json_encode($rspta);
        break;

    case 'mostrarAnular':
        $id = isset($_POST["id_mostrar_anular"]) ? limpiarCadena($_POST["id_mostrar_anular"]) : "";
        $rspta = $fac->mostrarAnular($id);
        echo json_encode($rspta);
        break;

    case 'facImpreso':
        $id = isset($_POST["id_fac_impreso"]) ? limpiarCadena($_POST["id_fac_impreso"]) : "";
        $rspta = $fac->facImpreso($id);
        echo json_encode($rspta);
        break;
