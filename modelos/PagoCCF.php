...
Class PagoCCF{
    ...
    public function insertar($fecha_abono,$comentario,$saldo,$abono,$nuevo_saldo,$id_forma_pago,$id_banco,$id_ccf,$id_user){
        $sql = "INSERT INTO tb_pago_cliente(fecha_abono,comentario,saldo,abono,nuevo_saldo,estado,id_forma_pago,id_banco,id_ccf,id_user) 
                VALUES ('$fecha_abono','$comentario',$saldo,$abono,$nuevo_saldo,1,$id_forma_pago,$id_banco,$id_ccf,$id_user)";
    ...
    public function listar(){
        $sql = "SELECT a.*, b.condicion, c.cliente, 
                IFNULL(total_general - (SELECT SUM(abono) FROM tb_pago_cliente as d WHERE d.id_ccf = a.id), total_general) as saldo_pendiente
                FROM tb_ccf as a 
    ...
    public function mostrar($id){
        $sql = "SELECT a.*, b.condicion, c.cliente, IFNULL((total_general - SUM(abono)), total_general) as saldo_pendiente
                    ...
                    INNER JOIN tb_condicion_pago as b ON a.id_condicion_pago = b.id
                    INNER JOIN tb_cliente as c ON a.id_cliente = c.id 
                    INNER JOIN tb_pago_cliente as d ON a.id = d.id_ccf
                    WHERE a.id = $id";
    ...
    {
        $sql ="UPDATE tb_ccf SET estado=3 WHERE id=$id";
        return ejecutarConsulta($sql);
    }
