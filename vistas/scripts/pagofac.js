...
function init() {
    ...
    $("#formulario").on("submit", function (e) {
     ...
     $.post("../ajax/pagofac.php?op=selectFormaPago", function (r) {
        $("#id_forma_pago").html(r);
        $('#id_forma_pago').selectpicker('refresh');
    })

     //Cargamos los items al select banco
     $.post("../ajax/pagofac.php?op=selectBanco", function (r) {
        $("#id_banco").html(r);
...
function listar() {
    tabla = $('#tbllistado').dataTable({
        ...
        "aServerSide": true,
        "ajax": {
            url: '../ajax/pagofac.php?op=listar',
            type: "get",
            dataType: "json",
            error: function (e) {
...
function mostrar(id){
    $.post("../ajax/pagofac.php?op=mostrar", {id: id}, function (data, status) {
...
function guardaryeditar(e) {
    ...
    $.ajax({
        url: "../ajax/pagofac.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        success: function (datos) {
            ...
            {
               $.ajax({
                    url: '../ajax/pagofac.php?op=actualizarEstado',
                    type: "post",
                    data: {id:id_fac},
                    dataType: "json",
                    error: function (e) {
...
function mostrarTotalPendiente() {
    $.post("../ajax/pagofac.php?op=mostrarTotalPendiente", function (data, status) {
        data = JSON.parse(data);
