...
function listarDetalle(id) {
    tabla_detalle = $('#tbllistado_detalle').dataTable({
        ...
        "aServerSide": true,
        "ajax": {
            url: '../ajax/ccf.php?op=listarDetalle',
            type: "post",
            data: {id: id},
            dataType: "json",
            error: function (e) {
...
function guardaryeditar(e) {
    ...
    $.ajax({
        url: "../ajax/ccf.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
        success: function (datos) {
            ...
            mostrarForm(true);
            tabla.ajax.reload();
            window.open('../ajax/printCCF.php?id='+id,'_blank');
            window.open('../ajax/printProformaCCF.php?id='+id,'_blank');
        }
    });
    //limpiar();
}
...
function guardaryeditarDetalle(e) {
    ...
    $.ajax({
        url: "../ajax/detalleCCF.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function eliminarDetalle(id) {
    alertify.confirm('Confirmación', '¿Seguro que desea eliminar?', function () {
            $.post("../ajax/detalleCCF.php?op=eliminar", {id: id}, function (e) {
                if (e == 1) {
...
function mostrarDetalle(id) {
    $.post("../ajax/detalleCCF.php?op=mostrar", {id: id}, function (data, status) {
...
function nuevo() {
    ...
    $.post("../ajax/ccf.php?op=nuevo", function (data, status) {
        data = JSON.parse(data);
...
function totalesCCF(id) {
    $.post("../ajax/ccf.php?op=totalesCCF", {id: id}, function (data, status) {
        ...
        var total = ventas+parseFloat(iva)+parseFloat(ajena);
...
/*function imprimirCCF(id) {
    //NO FUNCA :(
    $.ajax({
        type:"POST", // la variable type guarda el tipo de la peticion GET,POST,..
        url:"../ajax/printCCF.php", //url guarda la ruta hacia donde se hace la peticion
...
function  mostrarAnular(id) {
        ...
        $("#div_btn_anular").hide();
        $.post("../ajax/ccf.php?op=mostrarAnular", {id_mostrar_anular: id}, function (data, status) {
        var data = JSON.parse(data);
...
function  ccfImpreso() {
    var id = $("#id").val();
    //alert(id);
    $.post("../ajax/ccf.php?op=ccfImpreso", {id_ccf_impreso: id}, function (e) {
        if (e) {
...
function mostrarCCF(id) {
    $.post("../ajax/ccf.php?op=mostrarCCF", {id: id}, function (data, status) {
        data = JSON.parse(data);
        mostrarForm(true);
        $("#codigo").val(data.codigo);
